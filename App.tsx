    ...
    >
      <div className={`z-20 w-12 h-12 shrink-0 rounded-full flex items-center justify-center shadow-lg transition-transform duration-300 group-hover:scale-110 ${
        isFinal 
          ...
          : 'bg-white/[0.07] backdrop-blur-xl border border-white/10 border-primary/50'
      }`}>
        <motion.div 
...
export default function EngineeringWorkflow() {
  ...
  useEffect(() => {
    ...
    const updateStats = () => {
      setStats(prev => ({
        ...
        // Increased increment for visible progress bar movement
        progress: (prev.progress + 0.5 + Math.random() * 1.5) % 100,
        activeMonth: Math.floor((Date.now() / 1000) % 36) + 1
      }));
      
      // Recursive timeout for organic feel
  ...
  return (
    <div className="w-full h-full aspect-square flex items-center justify-center bg-slate-900 p-4">
      <div className="w-full h-full max-w-[600px] max-h-[600px] relative overflow-hidden rounded-xl bg-gradient-to-br from-sky-600 via-blue-600 to-indigo-700 text-white shadow-2xl flex flex-col">
        {/* Background Layers */}
        <div 
        ...
        <div className="relative z-10 h-full flex flex-col p-6 overflow-hidden">
          ...
          {/* Workflow Steps - Scrollable if content exceeds height */}
          ...
          >
            <button className="flex-1 bg-primary hover:bg-primary/90 py-3 rounded-lg text-[11px] font-bold uppercase transition-all shadow-lg shadow-primary/20 flex items-center justify-center gap-2">
              Schedule Tech Audit
            </button>
